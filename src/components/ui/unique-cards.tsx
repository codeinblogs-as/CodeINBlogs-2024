    ...
    return (
        <div className="mt-0">
            <div className="max-w-screen-xl mx-auto px-4 text-zinc-200 md:px-8">
                <div className="mt-0">
                    <ul className="grid gap-y-8 gap-x-12 sm:grid-cols-2 lg:grid-cols-3" id='cards-container'>
                        {features.map((item, idx) => (
                            <MagicalCard key={idx} className='flex'>
                                <li className="flex flex-col flex-grow bg-gradient-to-r from-indigo-950/10 to-violet-950/10 border border-[#323232]/50 bg-opacity-20 py-10 px-6 backdrop-blur-3xl rounded-lg">
                                <div className="flex-none w-12 h-12 bg-[#13131f]/20 rounded-lg flex items-center justify-center p-2 my-2">
                                    {item.icon}
                                </div>
                                <div>
